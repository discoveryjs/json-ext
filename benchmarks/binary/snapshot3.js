...
const MAX_UINT_23 = 0x007fffff;
const MAX_UINT_24 = 0x00ffffff;
const MAX_UINT_28 = 0x0fffffff;
const MAX_UINT_30 = 0x3fffffff;
...
const TYPE_STRING = 2;
const TYPE_UINT_8 = 3;       // uint8                  [0 ... 255 (0xff)]
const TYPE_UINT_16 = 4;      // uint16 + max(prev) + 1 [256 (0x100) ... 65791 (0x100ff)]
const TYPE_UINT_24 = 5;      // uint24 + max(prev) + 1 [65792 (0x10100] ... 16843007 (0x10100ff)]
const TYPE_UINT_32 = 6;      // uint32 + max(prev) + 1 [16843008 (0x1010100) ... 4311810303 (0x1010100ff)]
const TYPE_UINT_32_VAR = 7;  // [ 4311810304 (0x101010100) ... ]
...
class Writer {
    constructor(chunkSize = defaultChunkSize) {
        ...
        this.sb = 0;
        this.ss = 0;
        this.deflatedCount = 0;
        this.deflatedBetter = 0;
    }
    get value() {
        this.flushChunk();
    ...
    writeString(str) {
        ...
        this.writeVlq(byteLength << 1);
        this.sb += byteLength;

        // if (str.length > 1000) {
        //     const deflated = deflateRawSync(str);
        //     this.ss += deflated.byteLength;
        //     this.deflatedCount++;
        //     this.deflatedBetter += deflated.byteLength < byteLength;
        //     if (this.bytes.byteLength - this.pos > deflated.byteLength) {
        //         this.bytes.set(deflated, this.pos);
        //         this.pos += deflated.byteLength;
        //     } else {
        //         this.flushChunk();
        //         this.chunks.push(deflated);
        //         this.createChunk();
        //     }
        //     return;
        // } else {
    ...
    writeInt64(value) {
        ...
        this.view.setBigInt64(this.pos, BigInt(value));
        this.pos += 8;
    }
    writeUint8(value) {
        this.ensureCapacity(1);
    ...
    // The method writes at least 3 bytes (24bits)
    // The lower 16 bits of the number are stored as is. The rest of the number is stored
    // byte by byte, using 7 bits of each byte to store the number bits and 1 continuation bit
    writeUintVar24(value) {
        // Fast path for int30 numbers, it takes 3-4 bytes to encode a number
        if (value <= MAX_UINT_23) {
            // The number is fit into int23, store it as is, use 3 bytes
            this.ensureCapacity(3);
            this.view.setUint16(this.pos, value & MAX_UINT_16);
            this.view.setUint8(this.pos + 2, value >> 16);
            this.pos += 3;
        } else {
            const isInt31orMore = value > MAX_UINT_30 ? 0x80000000 : 0;
            console.log(value.toString(16), MAX_UINT_30.toString(16), isInt31orMore);

            // The number contains at least 30bits, use 4 bytes and set
            // a continuation bit for the 2nd byte, and for 3rd byte if the number
            // xxaaaaaa abbbbbbb bbbbbbbb bbbbbbbb
            // for int30
            // 0aaaaaaa 1bbbbbbb bbbbbbbb bbbbbbbb
            // for int31-64
            // 1aaaaaaa 1bbbbbbb bbbbbbbb bbbbbbbb
            this.writeUint32(
                isInt31orMore | ((value << 1) & 0x7f000000) |
                0x00800000 | (value & 0x7fffff)
            );

            if (isInt31orMore) {
                value = (value - (value & 0x3fffffff)) / 0x40000000;
                // write the rest part byte by byte
                do {
                    const payload = value & 0x7f;
                    value = (value - payload) / 0x80;

                    this.writeUint8(((value > 0) << 7) | payload);
                } while (value > 0);
            }
        }
    }
...
function encode(input, options = {}) {
    function writeReference(stringCluster, value) {
        const ref = stringCluster.get(value);
        if (ref !== undefined) {
    ...
    function writeString(value) {
        ...
        if (!writeReference(stringCluster, value)) {
            ...
            case TYPE_OBJECT: {
                ...
                while (true) {
                    ...
                    if (type & 1) {
                        ...
                        // definition
                        const key = readValue(TYPE_STRING);
            ...
            case TYPE_ARRAY: {
                ...
                if (hasObjectColumnKeys) {
                    ...
                    for (let i = 0; i < keysLength; i++) {
                        keys[i] = readValue(TYPE_STRING);
                    }

                    // read column values
                    for (let i = 0; i < keysLength; i++) {
                        const key = keys[i];
                        const vals = readValue(TYPE_ARRAY, objects.length);
